...
import type { UserListItem, UserDetail, UserCreatePayload, UserUpdatePayload, UserRole } from '@/services/users'
...
import { getStates, getCitiesByState, getStreetTypes } from '@/services/locations'
...
export const useUsersStore = defineStore('users', {
  ...
  getters: {
    ...
    currentAddressLabels(state): string {
      ...
      if (state.form.address.neighborhood) parts.push(state.form.address.neighborhood)
      return parts.join(' • ')
    },
    rules() {
      return {
  ...
  actions: {
    ...
    async openEdit(id: string) {
      ...
      try {
        ...
        if (acronym) {
          const st = this.states.find(s => String(s.acronym).toUpperCase() === String(acronym).toUpperCase())
    ...
    async submit() {
      ...
      try {
        if (this.isEdit && this.selectedUserId) {
          ...
          const contact: any = {}
          if (this.form.contact.country_code != null) contact.country_code = this.form.contact.country_code
          if (this.form.contact.ddd != null) contact.ddd = this.form.contact.ddd
          if (this.form.contact.phone_number) contact.phone_number = this.form.contact.phone_number
          if (Object.keys(contact).length) payload.contact = contact

          const address: any = {}
          if (this.form.address.street) address.street = this.form.address.street
          if (this.form.address.complement != null) address.complement = this.form.address.complement
          if (this.form.address.cep) address.cep = this.form.address.cep
          if (this.form.address.number != null) address.number = this.form.address.number
          if (this.form.address.neighborhood) address.neighborhood = this.form.address.neighborhood
          if (this.form.address.idCity) address.idCity = this.form.address.idCity
          if (this.form.address.idStreetType) address.idStreetType = this.form.address.idStreetType
          ...
          if (!this.form.role || !this.form.address.idCity || !this.form.address.idStreetType) {
